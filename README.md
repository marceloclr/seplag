...
- **Tela de senha** no estilo terminal. O campo *user* mostra `root` esmaecido e recebe o cursor verde do terminal; só a senha é exigida. A janela pulsa de leve até algo ser digitado; Enter com campos vazios também vibra. Erro treme a janela com borda vermelha; acerto pulsa em verde (a mesma vibração do sistema de comunicação). O fundo é um corredor de data center desenhado em canvas: LEDs brancos, verdes e laranja (status e atividade), poeira na luz, varredura no piso e aproximação lenta da câmera. O código guarda apenas o SHA-256 da senha, e a liberação dura enquanto o navegador estiver aberto (`sessionStorage`). **Sair**, no alto, volta à tela de senha. Atenção: num site estático, essa senha é uma cortina contra quem abre o link por acaso, não uma proteção real. O código é público e os sistemas continuam acessíveis pelos próprios endereços.
