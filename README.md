...
- **Tela de senha** no estilo terminal, com o título *Hub de Sistemas da COFIP / CAEXF* na barra da janela e, digitados no boot, os nomes da COFIP, da CAEXF e *Soluções de Gestão Financeira e Projetos*. O campo *usuário* mostra `root` esmaecido e recebe o cursor verde do terminal; só a senha é exigida. O campo de senha não é do tipo `password` (a senha fica só na memória do script e a tela mostra bolinhas), para que nenhum navegador ofereça salvar ou preencher a senha. A janela pulsa sem parar, alternando vermelho, amarelo, verde e azul (2,5 s por cor); Enter com campos vazios também vibra. Erro treme a janela com borda vermelha; acerto pulsa em verde (a mesma vibração do sistema de comunicação). No tema claro (padrão) o fundo é um meio-tom ardósia-lavanda, com as bolhas do cabeçalho do portal à deriva e uma malha de pontos, e o box é branco, elevado e com a faixa em degradê da barra do portal. No tema escuro o fundo é um degradê azul-noite. O botão sol/lua no canto da tela troca o tema. O código guarda apenas o SHA-256 da senha, e a liberação dura enquanto o navegador estiver aberto (`sessionStorage`). **Sair**, no alto, volta à tela de senha. Atenção: num site estático, essa senha é uma cortina contra quem abre o link por acaso, não uma proteção real. O código é público e os sistemas continuam acessíveis pelos próprios endereços.
...
- O tema claro é o padrão. O tema escolhido e as abas abertas ficam guardados só neste navegador (`localStorage`, chave `gfp.portal`).
